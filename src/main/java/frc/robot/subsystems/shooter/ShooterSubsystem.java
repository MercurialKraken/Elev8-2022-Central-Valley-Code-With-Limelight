...
public class ShooterSubsystem extends SubsystemBase {
  ...
  public void setPow(double shoot) {
    if (shoot > ShooterConstants.deadband)
      this.shooter.set(0.0);
    else
      this.shooter.set(shoot);
  }

  public void setSpeed(double speed) {
    double speedmotor = shootEncoder.getVelocity();
    if (Math.abs(speedmotor) < ShooterConstants.deadbandVelocity) {
      this.shootPIDController.setReference(speed, CANSparkMax.ControlType.kVelocity);
      ...
      // ShooterSubsystem.shootEncoder.getVelocity());
    } else {
      this.shooter.set(0);
      SmartDashboard.putBoolean("shooter enabled?", false);
    }
